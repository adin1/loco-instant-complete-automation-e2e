...
async function main() {
    ...
    const tenant = await db.query(`INSERT INTO tenants (code, name, tz)
    ...
    for (const s of services) {
        await db.query(`INSERT INTO services (tenant_id, slug, name)
       VALUES ($1, $2, $3)
       ON CONFLICT (slug) DO NOTHING;`, [tenantId, s, s.charAt(0).toUpperCase() + s.slice(1)]);
    }
    const users = [
    ...
    for (const u of users) {
        await db.query(`INSERT INTO users (tenant_id, role, phone_e164, email, password_hash)
       VALUES ($1, $2, $3, $4, 'demo_hash')
       ON CONFLICT (email) DO NOTHING;`, [tenantId, u.role, u.phone, u.email]);
    }
    const providers = await db.query(`INSERT INTO providers (tenant_id, user_id, display_name, is_verified, rating_avg, rating_count)
     SELECT $1, id,
            CASE
     ...
     RETURNING id, display_name;`, [tenantId]);
    for (const p of providers.rows) {
        await db.query(`INSERT INTO order_events (tenant_id, order_id, event_type, payload)
    ...
    const customer = await db.query(`SELECT id FROM users WHERE email='ana@client.ro' LIMIT 1;`);
    const customerId = customer.rows[0].id;
    const providerId = providers.rows[0].id;
    await db.query(`INSERT INTO orders (tenant_id, customer_id, provider_id, service_id, status, price_estimate, origin_geom)
     VALUES ($1, $2, $3, (SELECT id FROM services WHERE slug='croitorie'),
             'assigned', 120, ST_GeogFromText('SRID=4326;POINT(23.62 46.77)'));`, [tenantId, customerId, providerId]);
...
//# sourceMappingURL=seed-demo.backup.js.map